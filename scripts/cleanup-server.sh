...
note() {
  printf '\n# 설명: AWS 명령 외의 삭제 대상, 생략 항목 또는 로컬 정리 결과를 기록한다.\n' | tee -a "$1"
...
verify_zero() {
  local label="$1" description="$2" display="$3" count
  shift 3
  count="$(capture "$AFTER_LOG" "$description" "$display" "$@")"
...
if (( INSTANCE_OK )); then
  ...
  if [[ "$STATE" != terminated ]]; then
    logged "$BEFORE_LOG" '실습 EC2 인스턴스의 종료를 요청한다.' \
      "aws ec2 terminate-instances --instance-ids $INSTANCE_ID" \
      aws_ec2 terminate-instances --instance-ids "$INSTANCE_ID"
    logged "$BEFORE_LOG" 'EC2 인스턴스가 완전히 종료될 때까지 기다린다.' \
...
if (( VOLUME_OK )); then
  ...
  case "$STATE" in
    available)
      logged "$BEFORE_LOG" 'EC2 종료 후 남아 있는 실습 EBS 볼륨을 삭제한다.' \
        "aws ec2 delete-volume --volume-id $VOLUME_ID" aws_ec2 delete-volume --volume-id "$VOLUME_ID"
      logged "$BEFORE_LOG" 'EBS 볼륨 삭제가 완료될 때까지 기다린다.' \
        "aws ec2 wait volume-deleted --volume-ids $VOLUME_ID" aws_ec2 wait volume-deleted --volume-ids "$VOLUME_ID"
      ;;
    deleting)
      logged "$BEFORE_LOG" '이미 삭제 중인 EBS 볼륨이 완전히 삭제될 때까지 기다린다.' \
        "aws ec2 wait volume-deleted --volume-ids $VOLUME_ID" aws_ec2 wait volume-deleted --volume-ids "$VOLUME_ID"
      ;;
...
if (( KEY_OK )); then
  logged "$BEFORE_LOG" '실습 EC2 접속에 사용한 AWS Key Pair 메타데이터를 삭제한다.' \
    "aws ec2 delete-key-pair --key-name $LAB_KEY_NAME" aws_ec2 delete-key-pair --key-name "$LAB_KEY_NAME"
fi
...
if (( SG_OK )); then
  ...
  for attempt in {1..12}; do
    ...
    set -e
    {
      printf '\n# 설명: EC2 네트워크 인터페이스 종속성이 해제된 뒤 Security Group을 삭제한다.\n'
      printf '[%s] $ aws ec2 delete-security-group --group-id %s <attempt %d/12>\n' \
...
if (( ROUTE_OK )); then
  ...
  if [[ -n "$ASSOCIATION_ID" && "$ASSOCIATION_ID" != None ]]; then
    logged "$BEFORE_LOG" 'Public Subnet과 사용자 Route Table의 연결을 해제한다.' \
  ...
  logged "$BEFORE_LOG" '연결이 해제된 실습 Route Table을 삭제한다.' \
    "aws ec2 delete-route-table --route-table-id $ROUTE_ID" \
    aws_ec2 delete-route-table --route-table-id "$ROUTE_ID"
fi

if (( SUBNET_OK )); then
  logged "$BEFORE_LOG" '실습 Public Subnet을 삭제한다.' \
    "aws ec2 delete-subnet --subnet-id $SUBNET_ID" aws_ec2 delete-subnet --subnet-id "$SUBNET_ID"
fi
if (( IGW_OK )); then
  ...
  if [[ "$ATTACHED_VPC" == "$VPC_ID" ]]; then
    logged "$BEFORE_LOG" 'Internet Gateway와 실습 VPC의 연결을 해제한다.' \
      "aws ec2 detach-internet-gateway --internet-gateway-id $IGW_ID --vpc-id $VPC_ID" \
      aws_ec2 detach-internet-gateway --internet-gateway-id "$IGW_ID" --vpc-id "$VPC_ID"
  fi
  logged "$BEFORE_LOG" 'VPC에서 분리된 Internet Gateway를 삭제한다.' \
    "aws ec2 delete-internet-gateway --internet-gateway-id $IGW_ID" \
    aws_ec2 delete-internet-gateway --internet-gateway-id "$IGW_ID"
fi
if (( VPC_OK )); then
  logged "$BEFORE_LOG" '하위 네트워크 리소스가 제거된 실습 VPC를 삭제한다.' \
    "aws ec2 delete-vpc --vpc-id $VPC_ID" aws_ec2 delete-vpc --vpc-id "$VPC_ID"
fi

# 생성한 모든 리소스와 미사용 EIP가 남지 않았는지 태그 기준으로 최종 확인한다.
verify_zero 'active EC2' '활성 상태의 실습 EC2 인스턴스가 남아 있지 않은지 확인한다.' \
  'aws ec2 describe-instances <active project count>' \
  aws_ec2 describe-instances --filters "Name=tag:Project,Values=$LAB_PROJECT" \
    'Name=instance-state-name,Values=pending,running,shutting-down,stopping,stopped' \
    --query 'length(Reservations[].Instances[])' --output text
verify_zero EBS '실습 태그가 붙은 EBS 볼륨이 남아 있지 않은지 확인한다.' \
  'aws ec2 describe-volumes <project count>' \
  aws_ec2 describe-volumes --filters "Name=tag:Project,Values=$LAB_PROJECT" --query 'length(Volumes)' --output text
verify_zero EIP '실습 태그가 붙은 Elastic IP가 남아 있지 않은지 확인한다.' \
  'aws ec2 describe-addresses <project count>' \
  aws_ec2 describe-addresses --filters "Name=tag:Project,Values=$LAB_PROJECT" --query 'length(Addresses)' --output text
verify_zero 'Key Pair' '실습 태그가 붙은 AWS Key Pair가 남아 있지 않은지 확인한다.' \
  'aws ec2 describe-key-pairs <project count>' \
  aws_ec2 describe-key-pairs --filters "Name=tag:Project,Values=$LAB_PROJECT" --query 'length(KeyPairs)' --output text
verify_zero SG '실습 태그가 붙은 Security Group이 남아 있지 않은지 확인한다.' \
  'aws ec2 describe-security-groups <project count>' \
  aws_ec2 describe-security-groups --filters "Name=tag:Project,Values=$LAB_PROJECT" --query 'length(SecurityGroups)' --output text
verify_zero 'Route Table' '실습 태그가 붙은 Route Table이 남아 있지 않은지 확인한다.' \
  'aws ec2 describe-route-tables <project count>' \
  aws_ec2 describe-route-tables --filters "Name=tag:Project,Values=$LAB_PROJECT" --query 'length(RouteTables)' --output text
verify_zero Subnet '실습 태그가 붙은 Subnet이 남아 있지 않은지 확인한다.' \
  'aws ec2 describe-subnets <project count>' \
  aws_ec2 describe-subnets --filters "Name=tag:Project,Values=$LAB_PROJECT" --query 'length(Subnets)' --output text
verify_zero IGW '실습 태그가 붙은 Internet Gateway가 남아 있지 않은지 확인한다.' \
  'aws ec2 describe-internet-gateways <project count>' \
  aws_ec2 describe-internet-gateways --filters "Name=tag:Project,Values=$LAB_PROJECT" --query 'length(InternetGateways)' --output text
verify_zero VPC '실습 태그가 붙은 VPC가 남아 있지 않은지 확인한다.' \
  'aws ec2 describe-vpcs <project count>' \
