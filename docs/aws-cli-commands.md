...
> 모든 제출 증거를 확보한 뒤 `./scripts/cleanup-server.sh`를 실행한다. 스크립트는 삭제
> 대상과 프로젝트 태그를 표시하고, `codyssey-06-1` 확인 문구가 일치할 때만 삭제한다.
> 아래 명령은 자동화 스크립트가 수행하는 삭제 순서와 개별 명령을 검토하기 위한 참고 자료다.
